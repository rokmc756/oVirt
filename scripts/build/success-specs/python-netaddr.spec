...
%if 0%{?rhel} < 9 
# sphinx is not available on el9
BuildRequires:  python3-sphinx
%endif
...
%package -n python3.11-netaddr
Summary: A pure Python network address representation and manipulation library
BuildRequires:  python3.11-devel
BuildRequires:  python3.11-setuptools
# BuildRequires:  python3.11-pytest
%description -n python3.11-netaddr
%{desc}

%package -n python3.11-netaddr-shell
Summary: An IPython-based shell environment for the netaddr library
Requires:  python3.11-netaddr = %{version}-%{release}
%description -n python3.11-netaddr-shell
An IPython-based shell environment for the netaddr library
%endif
...
%if 0%{?rhel} >= 9
%files -n python3.11-netaddr
%define python3_pkgversion 3.11
%define __python3 /usr/bin/python3.11
%license COPYRIGHT
%doc AUTHORS CHANGELOG
%{python3_sitelib}/*netaddr
%{python3_sitelib}/*netaddr-%{version}-py?.??.egg-info
%endif
...
%if 0%{?rhel} >= 9
%files -n python3.11-netaddr-shell
%define python3_pkgversion 3.11
%define __python3 /usr/bin/python3.11
%{_bindir}/netaddr
%endif
...
%changelog
* Thu Mar 2 2023 Martin Necas <mnecas@redhat.com> - 0.8.0-12.1
